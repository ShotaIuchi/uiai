...
**Webテストはブラウザインスタンスが独立するため、複数シナリオを並列実行すること。**

```
# Phase 1: 全シナリオの runner を並列実行（1回のレスポンスで全 Task を同時呼出）
parallel:
  Task tool → web-test-runner (scenario_1)
  Task tool → web-test-runner (scenario_2)
  Task tool → web-test-runner (scenario_3)

# Phase 2: 全シナリオの evaluator を並列実行
parallel:
  Task tool → web-test-evaluator (scenario_1 results)
  Task tool → web-test-evaluator (scenario_2 results)
  Task tool → web-test-evaluator (scenario_3 results)
```

並列実行のメリット：
- コンテキスト分離（シナリオ間の状態汚染を防止）
- エラー隔離（1シナリオの失敗が他に影響しない）
- **所要時間 = 最も遅いシナリオの時間**（直列の場合は全シナリオの合計時間）
...
└── results/
    └── <timestamp>/
        ...
        └── <scenario-name>/
            ...
            ├── step_01_dom.html    # DOM スナップショット（要素特定が必要なステップのみ）
            └── step_01_a11y.json   # アクセシビリティツリー（要素特定が必要なステップのみ）
```
